...
<task type="auto">
  ...
  <action>
Build U-Boot with the updated rkbin blobs in two stages:

**Stage 1: Build rkbin dependency and verify blobs downloaded**

First, ensure rkbin downloads with the correct blobs:
```bash
make local-talos-sbc-rk3588-mainline DEST=_out 2>&1 | head -100
```

Then verify the v1.18 DDR and v1.48 BL31 blobs exist:
```bash
ls -la _out/libs/rkbin/bin/rk35/ | grep -E "v1\\.18|v1\\.48"
```
...
If these files do NOT exist, the rkbin commit download failed - do not proceed with U-Boot build.

**Stage 2: Complete U-Boot build**

If blobs verified, complete the build:
```bash
make local-talos-sbc-rk3588-mainline DEST=_out
```

This will:
1. Use the already-downloaded rkbin with v1.18/v1.48 blobs
...
Verify the build completes successfully and the binary is produced.
  ...
  <verify>
```bash
# Verify blobs were downloaded correctly
ls _out/libs/rkbin/bin/rk35/ | grep -E "v1\.18|v1\.48"

# Verify U-Boot binary exists
...
```
Blob check must show v1.18 DDR and v1.48 BL31 files. Binary should exist and be recognized as data/ARM binary.
  </verify>
  <done>u-boot-rockchip.bin built with DDR v1.18 and BL31 v1.48 (verified blobs exist in _out/libs/rkbin/bin/rk35/)</done>
</task>
...
4. Power on and observe for 2 minutes:
   ...
   **Understanding Observable Indicators:**
   - **LED activity** = DDR training and SPL success (primary early indicator)
     - LED blink within 0-10 seconds means DDR initialized and SPL is running
     - No LED activity means DDR training failed (same failure mode as attempts 1-3)
   - **HDMI output** = Full boot chain succeeded and kernel reached display driver
     - U-Boot on RK3588 has NO HDMI driver - it cannot output video
     - HDMI signal only appears if: DDR OK -> SPL OK -> U-Boot OK -> Kernel loads -> Display driver initializes
     - Any HDMI output means the entire boot chain worked

   **Timeline:**
   - 0-10 seconds: Watch LEDs (GPIO1_A4 = SYS, GPIO1_A6 = USER) for DDR/SPL success
   - 10-30 seconds: If LEDs blinked, U-Boot is likely running (no video expected)
   - 30-120 seconds: Watch for HDMI signal (only if kernel boots) and network activity

5. Report results:
   - Did LEDs blink? (Y/N) - This is the key DDR success indicator
   - Did HDMI show anything? (Y/N) - Only expected if full kernel boot
...
- "partial" - Some improvement but not full boot
  ...
  <done>Attempt #4 boot test completed and documented in BOOT-TEST-CHECKLIST.md with observable indicators recorded (LED activity/HDMI output/network status)</done>
</task>
...
3. rkbin blobs verified: v1.18 DDR and v1.48 BL31 exist in _out/libs/rkbin/bin/rk35/
4. U-Boot builds successfully with updated blobs
5. Boot test Attempt #4 conducted with results documented
