...
must_haves:
  ...
  key_links:
    - from: ".github/workflows/ci.yaml"
      to: "Docker Hub"
      via: "docker/login-action with DOCKERHUB_TOKEN secret"
      pattern: "docker.io.*123417"
---
...
<task type="auto">
  <name>Task 1: Add Docker Hub authentication to CI workflow</name>
...
Keep the existing GHCR login for backward compatibility with existing boards.
  </action>
  <verify>Workflow YAML is valid: `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yaml'))"`</verify>
  <done>Docker Hub login step added to sbc-rk3588 job, env vars configured</done>
</task>

<task type="auto">
  <name>Task 2: Add nanopi-m6 to board matrix and configure Docker Hub push</name>
  <files>.github/workflows/ci.yaml</files>
  <action>
...
- name: Build flashable image
  ...
  run: |
    ...
    metal --arch arm64 \
      --overlay-image=${OVERLAY_REGISTRY}${OVERLAY_REPO}/talos-sbc-${{ matrix.board.registry == 'dockerhub' && 'nanopi-m6' || 'rk3588' }}-${{ matrix.kernel }}:${{ env.SBC_RK3588_TAG }} \
      --overlay-name=rk3588 \
...
2. Docker Hub login step present with correct secret names
3. nanopi-m6 in board matrix with chipset rk3588s
4. Docker Hub push logic for nanopi-m6 only
5. Existing rock-5a/rock-5b GHCR logic unchanged
6. Tag trigger pattern supports v*-nanopi-m6 format
</verification>
...
- Workflow will push nanopi-m6 overlay to docker.io/123417/talos-sbc-nanopi-m6
